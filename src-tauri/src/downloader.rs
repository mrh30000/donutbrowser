...
mod tests {
  ...
  fn test_clear_download_state_for_browser_removes_stuck_keys() {
    ...
    let key = "chrome-1.2.3-resolved".to_string();
    {
      let mut downloading = DOWNLOADING_BROWSERS.lock().unwrap();
    ...
    clear_download_state_for_browser("chrome");

    assert!(
      !is_downloading("chrome", "1.2.3-resolved"),
      "stuck chrome key should be cleared even when version differs from request"
    );
    {
      let tokens = DOWNLOAD_CANCELLATION_TOKENS.lock().unwrap();
      assert!(
        !tokens.contains_key(&key),
        "stuck chrome cancellation token should be cleared"
      );
    }
    assert!(
