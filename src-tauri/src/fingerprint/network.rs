...
pub fn ssl_to_firefox_prefs(config: &SslConfig) -> HashMap<String, serde_json::Value> {
  ...
  for version in &config.disabled_versions {
    match version.as_str() {
      "SSL3" | "ssl3" => {
        prefs.insert("security.tls.version.min".to_string(), serde_json::json!(1));
      }
      "TLS1.0" | "tls1.0" => {
        prefs.insert("security.tls.version.min".to_string(), serde_json::json!(2));
      }
      "TLS1.1" | "tls1.1" => {
        prefs.insert("security.tls.version.min".to_string(), serde_json::json!(3));
      }
      _ => {}
    }
  }
...
mod tests {
  ...
  fn test_ssl3_disabled_sets_min_version() {
    ...
    let prefs = ssl_to_firefox_prefs(&config);
    assert_eq!(
      prefs.get("security.tls.version.min"),
