...
impl BrowserRunner {
  ...
  pub async fn launch_browser_with_debugging(
    ...
    // Camoufox starts (and PID-reconciles) its own local proxy
...
pub async fn launch_browser_profile_impl(
  ...
  // Launch browser or open URL in existing instance. Camoufox
  // starts its own local proxy inside `launch_browser_internal`; any
